...
```{r}
#This table is saying the % of people who answered exactly correctly by each cell (subset of data based on row/column intersection, so the value is based on total number of people who belong to x perceived health category and y income bracket)
...
#This table is saying the % of people who answered correctly within +/-1 category by each cell (subset of data based on row/column intersection, so the value is based on total number of people who belong to x perceived health category and y income bracket)
finaltab2<-matrix(NA, nrow=5, ncol=7)
for(i in 1:length(unique(valid_data_df$INCGRP_A))){
  for(x in 1:length(unique(valid_data_df$PHSTAT_A))){
    test<-valid_data_df$percep_true_2[valid_data_df$INCGRP_A==i & valid_data_df$PHSTAT_A==unique(sort(valid_data_df$PHSTAT_A, decreasing=F))[x]] 
    vec1<-sum(test)/length(test)
    finaltab2[i,x]<-vec1
  }
}
